	...
	* */
	...
	private async _loadBoardsTexture(): Promise<void> {
		...
		for (const key in this.texture_boards) {
			...
			const texture_aspect_ratio = texture.image.height / texture.image.width;
			let scale_x = 1;
			let scale_y = 1;

			if (texture_aspect_ratio > 1) {
				scale_x = texture_aspect_ratio;
			} else {
				scale_y = 1 / texture_aspect_ratio;
			}
	...
	* */
	private _configureGallery() {
		for (const key in this.texture_boards) {
			...
			board.userData = {
				name: board.name,
				title: BOARDS_INFO[key].author + "的作品",
				author: BOARDS_INFO[key].author,
				describe: BOARDS_INFO[key].author + "的作品",
				index: key,
